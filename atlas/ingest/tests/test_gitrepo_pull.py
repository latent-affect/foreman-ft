"""GOALS.json C13-C17 (gitrepo_to_ingest amendment). Run from the repo root:
    ...
    /path/to/venv/bin/python3 -m unittest atlas.ingest.tests.test_gitrepo_pull -v
...
class TicketCandidateRegexTests(unittest.TestCase):
    ...
    def test_a_real_registered_style_prefix_is_extracted(self):
        self.assertEqual(
            gitrepo_pull.extract_ticket_candidates("Fix override widening (DEMOPROJ-9)"),
            ["DEMOPROJ-9"],
        )
...
class InsertAndValidationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.db.conn.execute(
            ...
            "root_state, refreshed_at) VALUES ('DEMOPROJ', 'DEMOPROJ', '/x', 'git-repo', "
    ...
    def test_a_registered_prefix_candidate_becomes_a_real_ticket_link(self):
        commit = {
            ...
            "author_raw": "someone@example.invalid", "subject": "Fix thing (DEMOPROJ-9)",
            "files": [("x.py", 3, 1)],
        }
        gitrepo_pull.insert_commit(self.db.conn, "DEMOPROJ", commit, self.db.run_id, {"DEMOPROJ"})
        self.db.conn.commit()
        row = self.db.conn.execute(
            "SELECT ticket_id, origin FROM git_commit_ticket WHERE sha = ?", (commit["sha"],)
        ).fetchone()
        self.assertEqual(row, ("DEMOPROJ-9", "commit-message-regex"))
    ...
    def test_reinserting_the_same_commit_is_a_no_op_not_a_duplicate(self):
        commit = {
            ...
            "author_raw": "someone@example.invalid", "subject": "Idempotency check (DEMOPROJ-9)",
            "files": [("x.py", 1, 0)],
        }
        gitrepo_pull.insert_commit(self.db.conn, "DEMOPROJ", commit, self.db.run_id, {"DEMOPROJ"})
        gitrepo_pull.insert_commit(self.db.conn, "DEMOPROJ", commit, self.db.run_id, {"DEMOPROJ"})
...
class PullOrchestrationTests(unittest.TestCase):
    ...
    def test_pull_against_a_real_repo_ingests_every_commit_exactly_once_across_two_passes(self):
        ...
        n1 = gitrepo_pull.pull_git_commits(self.db.conn, self.repo.path, "DEMOPROJ", {"DEMOPROJ"}, self.db.run_id)
        self.assertEqual(n1, 2)
        self.repo.commit("c.txt", "3\n", "Third (DEMOPROJ-2)")
        n2 = gitrepo_pull.pull_git_commits(self.db.conn, self.repo.path, "DEMOPROJ", {"DEMOPROJ"}, self.db.run_id)
        ...
        self.assertEqual([r[0] for r in linked], ["DEMOPROJ-1", "DEMOPROJ-2"])
