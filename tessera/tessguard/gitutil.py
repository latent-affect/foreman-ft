...
def git_worktree_paths(cwd):
    """FORE-282 (propagated from DEVH-54's dev-harness-run2 fix). All worktree paths sharing
    cwd's repository (`git worktree list`), or [] if cwd isn't inside a git working tree or
    the command fails for any reason -- never raises, same convention as git_toplevel above. A
    git worktree has its own distinct top-level working directory (git_toplevel(cwd) returns
    that worktree's own path, not any sibling's), but every worktree of one repository shares
    the same object store and the same TESSERA project identity ought to apply to all of them
    -- this is what lets a caller try sibling paths when the current worktree's own path isn't
    itself a registered source_root."""
    proc = subprocess.run(
